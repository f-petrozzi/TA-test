...
### Future Improvements
1. Swap `SimpleMCPClient` with a hosted MCP server for true multi-agent workflows.
2. Add per-user OAuth so assistants act on behalf of individual staff.
3. Expand corpus ingestion pipeline with auto-diff + scheduled refreshes.
4. Build evaluation harness for RAG answer quality + guardrail coverage.
